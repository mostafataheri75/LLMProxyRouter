...
async def chat_completions(body: ChatCompletionRequest, request: Request):
    ...
    try:
        req_logger.log_request("POST", "/v1/chat/completions", model_name, server.url, payload)
        response = await forward_request(client, server, "/v1/chat/completions", payload)
        resp_data = response.json()
        req_logger.log_response(
            "/v1/chat/completions", model_name, server.url,
