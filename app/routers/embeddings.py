...
async def embeddings(body: EmbeddingRequest, request: Request):
    ...
    try:
        req_logger.log_request("POST", "/v1/embeddings", model_name, server.url, payload)
        response = await forward_request(client, server, "/v1/embeddings", payload)
        resp_data = response.json()
        req_logger.log_response(
            "/v1/embeddings", model_name, server.url,
